...
match engine directly. They resolve into a flat `TacticalModifiers` struct of multipliers/biases
(attack, midfield, defense, tempo, pressing-aggression) applied to Phase Strength. This is a
deliberate boundary: the match engine package has zero knowledge of formations or role names, so the
tactics ticket (and any future tactics rework) can change that vocabulary freely without touching the
match engine, as long as it still produces the same five numbers.
