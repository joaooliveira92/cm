...
Selection Filters restrict visible squad rows by team, position, availability, registration, age, fitness, morale, contract, transfer, and selection state without changing the underlying squad or lineup.

## 2. Primary user goals

- Edit filter criteria
- Preview matching count
- Apply without modifying hidden player state
- Save and restore personal filter presets
...
| FILTER SQUAD                                                                  |
|------------------------------------------------------------------------------|
| Team [First Team v] Position [All v] Availability [Available v]               |
| Registration [All v] Selection [All v] Age [Any v]                            |
| Matching players: 21                                                         |
| [Reset] [Save Preset] [Cancel] [Apply]                                       |
+------------------------------------------------------------------------------+
```
...
```typescript
interface SquadFilterDefinition {
  readonly filterId?: string;
  readonly managerId: string;
  readonly teamIds: readonly string[];
  readonly positionIds: readonly string[];
  readonly availabilityStates: readonly string[];
  readonly registrationStates: readonly string[];
  readonly selectionStates: readonly string[];
  readonly ageProfileId?: string;
...
- Filters never remove hidden selections
- A hidden selected-player notice is persistent
- Criteria use named profiles instead of arbitrary hidden thresholds
...
1. Filters never remove hidden selections
2. A hidden selected-player notice is persistent
3. Criteria use named profiles instead of arbitrary hidden thresholds
4. Result counts must not leak inaccessible players
...
```text
Implement Selection Filters for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify selection filters screen
```
