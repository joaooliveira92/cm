...
The Player Action Menu is the contextual command surface for selection, squad assignment, registration, transfer, contract, scouting, interaction, training, comparison, and navigation actions.

## 2. Primary user goals

- Open action groups
- Execute immediate reversible actions
- Open dedicated consequential workflows
- Apply actions to one or several selected players
- Inspect disabled-action reasons
...
```text
| PLAYER ACTIONS | Player Name |
| Squad: Move to First Team, Reserve, Youth |
| Selection: Add to XI, Bench, Remove |
| Contract and Transfer | Scouting | Interaction |
| Compare | Add to Shortlist | Open Profile |
```
...
```typescript
interface PlayerActionMenuModel {
  readonly playerId: string;
  readonly sourceContext: PlayerActionSourceContext;
  readonly groups: readonly PlayerActionGroup[];
...
- Actions must be generated authoritatively for the current context
- Disabled actions remain explainable
- Consequential actions require dedicated confirmation or workflow
- Duplicate actions require idempotency keys
...
1. Actions must be generated authoritatively for the current context
2. Disabled actions remain explainable
3. Consequential actions require dedicated confirmation or workflow
4. Duplicate actions require idempotency keys
...
```text
Implement Player Action Menu for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player action menu screen
```
