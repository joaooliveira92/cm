...
Player History presents the chronological record of clubs, loans, transfers, seasons, international teams, appearances, and major career milestones.

## 2. Primary user goals

- Switch between season table, timeline, transfers, and international history
- Open historical club, competition, or transfer
- Filter senior, reserve, youth, and international records
- Export a safe public history
...
```text
| PLAYER > HISTORY |
| Season Club Competition Apps Goals Rating |
| 2004/05 North United First Division 24 8 7.16 |
| Timeline: joined, loaned, transferred, capped, retired |
```
...
```typescript
interface PlayerHistoryModel {
  readonly playerId: string;
  readonly seasonRows: readonly PlayerSeasonHistoryRow[];
  readonly events: readonly PlayerHistoryEvent[];
  readonly internationalHistory: readonly InternationalHistoryRow[];
...
- History is append-only except audited corrections
- Loan and permanent employment periods remain distinct
- Unknown historical detail must not be fabricated
- Extinct entities require stable historical fallback labels
...
1. History is append-only except audited corrections
2. Loan and permanent employment periods remain distinct
3. Unknown historical detail must not be fabricated
4. Extinct entities require stable historical fallback labels
...
```text
Implement Player History for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player history screen
```
