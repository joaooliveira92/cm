...
Player Comparison compares selected players across identity, availability, positions, attributes, form, statistics, contracts, value, development, and knowledge confidence.

## 2. Primary user goals

- Add, remove, or swap comparison players
- Select comparison categories
- Open a player or metric definition
- Save a manager-private comparison preset
...
```text
| PLAYER COMPARISON | Player A | Player B |
| Position DC | DC | Age 25 | 28 |
| Pace 14 | 11 | Marking 15 | 16 |
| Form 7.10 | 6.84 | Contract 2007 | 2006 |
| Knowledge confidence shown per value |
```
...
```typescript
interface PlayerComparisonModel {
  readonly playerIds: readonly string[];
  readonly categoryIds: readonly string[];
  readonly columns: readonly PlayerComparisonColumn[];
  readonly rows: readonly PlayerComparisonMetric[];
...
- Compare only values visible for each player
- Unknown must not rank as zero
- Units and attribute scales require normalization
- Derived winner highlighting must include textual meaning and uncertainty
...
1. Compare only values visible for each player
2. Unknown must not rank as zero
3. Units and attribute scales require normalization
4. Derived winner highlighting must include textual meaning and uncertainty
...
```text
Implement Player Comparison for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player comparison screen
```
