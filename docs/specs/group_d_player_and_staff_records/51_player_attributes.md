...
Player Attributes presents technical, mental, physical, goalkeeping, and hidden-or-unavailable capability information according to the active manager’s knowledge and scouting confidence.

## 2. Primary user goals

- Switch attribute groups and display modes
- Inspect an attribute definition
- Change the observed development period
- Open coach or scout evidence supporting the view
...
```text
| PLAYER > ATTRIBUTES | Knowledge: Good |
| Technical | Mental | Physical | Goalkeeping |
| Passing 14 | Decisions 12 | Pace 15 | Handling - |
| Attribute changes: +1 / -1 indicators with period selector |
```
...
```typescript
interface PlayerAttributesModel {
  readonly playerId: string;
  readonly observedAt: string;
  readonly knowledge: KnowledgeConfidence;
  readonly groups: readonly AttributeGroupView[];
  readonly changeIndicators: readonly AttributeChangeIndicator[];
  readonly rangesUsed: boolean;
...
- Do not expose hidden exact values through sorting, colors, tooltips, or exports
- Low knowledge may display ranges or Unknown
- Attribute scale and bands are policy-defined
- Change indicators must distinguish observed change from certainty
...
1. Do not expose hidden exact values through sorting, colors, tooltips, or exports
2. Low knowledge may display ranges or Unknown
3. Attribute scale and bands are policy-defined
4. Change indicators must distinguish observed change from certainty
...
```text
Implement Player Attributes for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player attributes screen
```
