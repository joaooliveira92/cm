# Screen 268: Favorites and Pinned Items
...
Favorites and Pinned Items manages manager-private shortcuts to entities, searches, reports, fixtures, competitions, and workflows.

## 2. Primary user goals

- Pin or unpin supported items
- Create private groups and ordering
- Open a favorite in its valid current context
- Resolve missing or inaccessible targets
...
- Favorites store stable references, not copied private data
- Pinning does not grant access
- Ordering changes are presentation-only
- Deleted targets preserve a safe unavailable entry until removed
...
1. Favorites store stable references, not copied private data
2. Pinning does not grant access
3. Ordering changes are presentation-only
4. Deleted targets preserve a safe unavailable entry until removed
...
```text
Implement Favorites and Pinned Items for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify favorites and pinned items screen
```
