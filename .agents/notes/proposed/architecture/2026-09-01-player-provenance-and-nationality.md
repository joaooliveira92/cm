...
This constrains the open question of how much geography a `results-only` nation is worth: every player
originating from a nation with no city rows has a NULL birthplace, and that is part of the cost of
trimming those rows.
...
- `players.birth_city_id` is nullable, references the cities table, and is NULL for every player whose
  origin nation contributed no city rows to this save.
...
- **NULL birthplaces may be common.** If the world is usually generated with few nations loaded, most
  imported players will have no birth city, and a profile screen built assuming a city is present will
  look broken. The nationality fallback has to be designed in, not bolted on.
