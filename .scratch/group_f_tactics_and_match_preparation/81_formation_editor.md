...
The Formation Editor defines the spatial structure of a tactic by assigning valid position slots, lines, widths, role zones, and shape variants while preventing overlapping or unsupported configurations.

## 2. Primary user goals

- Move a slot between permitted tactical zones
- Add or remove slots within policy constraints
- Mirror a shape or restore a named formation
- Assign default roles to formation slots
- Preview and apply one atomic formation revision
...
```text
+------------------------------------------------------------------------------+
| FORMATION EDITOR                         Tactic: Balanced 4-4-2               |
|------------------------------------------------------------------------------|
|                     ST        ST                                            |
|             ML             MC       MC             MR                       |
|             DL             DC       DC             DR                       |
|                              GK                                              |
|------------------------------------------------------------------------------|
| Selected slot: Left central midfielder                                       |
| Position zone [MC v] | Default role [Central Midfielder v]                   |
| [Add Slot] [Remove Slot] [Mirror] [Restore Shape] [Apply]                    |
+------------------------------------------------------------------------------+
```
...
interface FormationDraft {
  ...
  readonly shapeProfileId: string;
  readonly phaseVariants?: readonly FormationPhaseVariant[];
  readonly constraints: FormationConstraints;
...
- A formation must satisfy goalkeeper, outfield-player, and slot-count policies
- Slots cannot overlap when the tactical model forbids overlap
- Changing shape must preserve compatible player instructions and flag incompatible ones
- Drag, keyboard, and menu operations must use the same validation path
...
1. A formation must satisfy goalkeeper, outfield-player, and slot-count policies
2. Slots cannot overlap when the tactical model forbids overlap
3. Changing shape must preserve compatible player instructions and flag incompatible ones
4. Drag, keyboard, and menu operations must use the same validation path
...
```text
Implement Formation Editor for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify formation editor screen
```
