# Screen 84: Individual Player Instructions
...
Individual Player Instructions customizes the selected player or tactical slot’s behavior, including movement, risk, pressing, marking, tackling, passing, crossing, dribbling, and positional freedom.

## 2. Primary user goals

- Set or clear an instruction override
- Return an instruction or all instructions to role defaults
- Copy compatible instructions to another slot
- Set a supported opposition marking target
- Apply one validated assignment revision
...
```text
+------------------------------------------------------------------------------+
| PLAYER INSTRUCTIONS: MORGAN RIGHT                  Slot: DR                   |
|------------------------------------------------------------------------------|
| Role baseline: Full Back | Duty: Support                                       |
| Overrides                                                                      |
| Passing risk [Default v] | Crossing [More Often v] | Pressing [Standard v]    |
| Marking target [None v]                                                        |
|------------------------------------------------------------------------------|
| [Use Role Defaults] [Copy Instructions] [Apply]                               |
+------------------------------------------------------------------------------+
```
...
```typescript
interface PlayerInstructionsDraft {
  readonly tacticId: string;
  readonly assignmentId: string;
  readonly playerId?: string;
  readonly slotId: string;
  readonly roleProfileId: string;
  readonly overrides: Readonly<Record<string, TacticalInstructionValue>>;
...
- Role defaults and explicit overrides remain distinguishable
- Player-specific instructions must not leak into another player after substitution unless slot-scoped by design
- Unsupported role and instruction combinations are rejected
- Copying previews incompatible or skipped instructions
...
1. Role defaults and explicit overrides remain distinguishable
2. Player-specific instructions must not leak into another player after substitution unless slot-scoped by design
3. Unsupported role and instruction combinations are rejected
4. Copying previews incompatible or skipped instructions
...
```text
Implement Individual Player Instructions for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify individual player instructions screen
```
