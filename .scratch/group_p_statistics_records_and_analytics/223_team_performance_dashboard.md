# Screen 223: Team Performance Dashboard
...
Team Performance Dashboard analyzes a club or national team across results, goals, chance creation, defending, possession, pressing, set pieces, discipline, workload, and competition context.

## 2. Primary user goals

- Choose team, competition, period, venue, and match-state scopes
- Inspect metric definitions and supporting matches
- Compare periods or selected teams
...
- Comparisons use compatible definitions and coverage
- Opponent strength adjustments are explicit when modeled
- Results-only matches are excluded from unsupported metrics
- Official results remain separate from analytical ratings
...
1. Comparisons use compatible definitions and coverage
2. Opponent strength adjustments are explicit when modeled
3. Results-only matches are excluded from unsupported metrics
4. Official results remain separate from analytical ratings
...
```text
Implement Team Performance Dashboard for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify team performance dashboard screen
```
