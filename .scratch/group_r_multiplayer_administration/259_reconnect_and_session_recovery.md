# Screen 259: Reconnect and Session Recovery
...
Reconnect and Session Recovery restores a disconnected participant through authenticated session re-entry, compatibility checks, snapshot or event replay, manager-control restoration, and conflict review.

## 2. Primary user goals

- Reconnect to the known session
- Review changed state and expired drafts
- Synchronize from a verified checkpoint
- Resume valid manager control after authoritative approval
...
- Unsubmitted local drafts are never applied silently
- Session credentials remain protected
- Late commands from the disconnected instance are rejected
- Recovery is resumable and idempotent
...
1. Unsubmitted local drafts are never applied silently
2. Session credentials remain protected
3. Late commands from the disconnected instance are rejected
4. Recovery is resumable and idempotent
...
```text
Implement Reconnect and Session Recovery for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify reconnect and session recovery screen
```
