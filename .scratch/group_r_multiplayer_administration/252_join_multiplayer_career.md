# Screen 252: Join Multiplayer Career
...
Join Multiplayer Career validates an invitation or session reference, identity, client version, content manifest, available manager slots, privacy terms, and synchronization prerequisites.

## 2. Primary user goals

- Enter or follow an authorized session reference
- Review session rules and compatibility
- Select an available participant identity or permitted manager slot
- Join, cancel, or retry synchronization
...
- Join credentials are never logged or displayed after use
- Compatibility is verified before career data is downloaded
- Joining does not grant manager authority automatically
- Rejected clients receive safe non-sensitive reasons
...
1. Join credentials are never logged or displayed after use
2. Compatibility is verified before career data is downloaded
3. Joining does not grant manager authority automatically
4. Rejected clients receive safe non-sensitive reasons
...
```text
Implement Join Multiplayer Career for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify join multiplayer career screen
```
