...
The Multiplayer Centre consolidates active sessions, invitations, recent careers, participant state, connectivity, synchronization, checkpoints, host status, and administrative alerts.

## 2. Primary user goals

- Open, create, join, or resume a multiplayer career
- Review participant, host, readiness, and synchronization state
- Open permissions, checkpoints, moderation, and session history
- Resolve reconnect, version, or content compatibility alerts
...
- Single-player and multiplayer careers remain distinct
- Private session metadata follows invitation policy
- The centre never exposes session secrets
- Connection state and simulation state are displayed separately
...
1. Single-player and multiplayer careers remain distinct
2. Private session metadata follows invitation policy
3. The centre never exposes session secrets
4. Connection state and simulation state are displayed separately
...
```text
Implement Multiplayer Centre for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify multiplayer centre screen
```
