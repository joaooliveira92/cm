...
7. **Status & data display** — (c) No CM-style status abbreviations (Inj, Sus, Lmp, etc.). Raw numeric attribute display only.

8. **Skins & theming** — (c) None. Single fixed palette, no CSS variables, no theme provider. Match-screen defines CSS custom properties but no runtime switching.

9. **Icons & visual markers** — (b) Emoji-only in match-screen component (⚽🟨🟥). Main screens are text-only. No icon library, no SVGs, no color-coded comparison.
