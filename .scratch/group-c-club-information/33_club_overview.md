...
The Club Overview is the primary landing page for a club. It combines current competitive position, upcoming match, recent form, squad availability, finances, board objectives, facilities, and actionable alerts into one permission-aware dashboard.

## 2. Primary user goals

- Open any club subsection
- Open the next fixture or recent result
- Resolve squad, registration, contract, or finance alerts
- Change the viewed club without changing the controlled club
- Return to the prior safe route
...
```text
+------------------------------------------------------------------------------+
| NORTH UNITED                                      First Division | 4th       |
|------------------------------------------------------------------------------|
| Next fixture        Recent form        Club status                           |
| Example City (H)    W D W L W          Professional                          |
|------------------------------------------------------------------------------|
| Squad availability  Board objectives   Finances                              |
| Available 23        Top-half finish    Transfer budget: Moderate             |
| Injured 3           Confidence: Stable Wage budget: Near limit               |
|------------------------------------------------------------------------------|
| [Squad] [Tactics] [Fixtures] [Transfers] [Finances] [Facilities]             |
+------------------------------------------------------------------------------+
```
...
```typescript
interface ClubOverviewModel {
  readonly clubId: string;
  readonly revision: number;
  readonly identity: ClubIdentitySummary;
  readonly competition: CompetitionPositionSummary;
  readonly nextFixture?: FixtureSummary;
  readonly recentForm: readonly ResultSummary[];
  readonly squadAvailability: SquadAvailabilitySummary;
  readonly financialPreview: ClubFinancialPreview;
  readonly objectives: readonly BoardObjectiveSummary[];
  readonly alerts: readonly ClubAlert[];
  readonly permittedActions: readonly ClubAction[];
}
```
...
- Overview figures must come from one coherent club revision
- Private budgets and staff data obey viewer permissions
- Alerts link to valid corrective workflows
- The page must remain useful when the club has no upcoming fixture
...
1. Overview figures must come from one coherent club revision
2. Private budgets and staff data obey viewer permissions
3. Alerts link to valid corrective workflows
4. The page must remain useful when the club has no upcoming fixture
...
```text
Implement Club Overview for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify club overview screen
```
