...
- [ ] Navigating away at full time and back still offers Accept result
- [ ] Accepting after that return commits the Matchday once
- [ ] A renderer test covers leave-and-return at full time
- [ ] Leaving while the commit is in flight (`committing`) is covered too: since ticket 08 no session
      is recorded at `committing`/`committed`, so that return also lands on a missing match
- [ ] Returning to Match day after an accepted result shows the next state, not Kick off for the
