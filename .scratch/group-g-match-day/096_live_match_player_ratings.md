# Screen 96: Live Match Player Ratings
...
Live Match Player Ratings shows current participating-player performance ratings, minutes, position, role, condition, major events, and substitution state during the match.

## 2. Primary user goals

- Sort by team, position, rating, condition, or events
- Open a player performance detail
- Select a controlled player for tactical or substitution review
- Compare current and prior-period ratings
...
```text
+------------------------------------------------------------------------------+
| Player ratings, position, role, condition, events, substitution and bench st |
|------------------------------------------------------------------------------|
...
```typescript
interface LivePlayerRatingsModel {
...
- Ratings are derived and can change until match completion
- Bench, unused, substituted, dismissed, and injured states are distinct
- Opponent ratings appear only when permitted
- Ratings must not expose hidden low-level event weights
...
1. Ratings are derived and can change until match completion
2. Bench, unused, substituted, dismissed, and injured states are distinct
3. Opponent ratings appear only when permitted
4. Ratings must not expose hidden low-level event weights
...
```text
Implement Live Match Player Ratings for an original football-management simulation. Use one
...
```text
docs(game-ui): specify live match player ratings screen
```
