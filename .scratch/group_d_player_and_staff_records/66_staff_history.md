...
Staff History presents chronological employment, roles, clubs, qualifications, achievements, managerial records, and major career events.

## 2. Primary user goals

- Switch employment, qualification, managerial, and timeline views
- Open historical club, competition, or role
- Filter period and role type
- Export a safe public summary
...
```text
| STAFF > HISTORY |
| Season Club Role Division Record |
| 2004/05 North United Assistant Manager |
| Timeline: appointed, promoted, qualified, resigned, retired |
```
...
```typescript
interface StaffHistoryModel {
  readonly staffId: string;
  readonly employmentRows: readonly StaffEmploymentHistoryRow[];
  readonly events: readonly StaffHistoryEvent[];
  readonly qualifications: readonly QualificationHistoryItem[];
  readonly managerialRecord?: ManagerialRecordSummary;
...
- History is append-only except audited correction
- Overlapping compatible roles must be modeled explicitly
- Managerial match records require consistent competition coverage
- Unknown legacy history must remain uncertain
...
1. History is append-only except audited correction
2. Overlapping compatible roles must be modeled explicitly
3. Managerial match records require consistent competition coverage
4. Unknown legacy history must remain uncertain
...
```text
Implement Staff History for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify staff history screen
```
