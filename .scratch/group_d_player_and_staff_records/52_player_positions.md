...
Player Positions visualizes natural, accomplished, competent, and unfamiliar playing positions, preferred sides, central roles, and role suitability without equating familiarity with current tactical selection.

## 2. Primary user goals

- Select a pitch zone for detail
- Review role suitability and evidence
- Open tactical assignment for the controlled club
- Compare positions with another player
...
```text
| PLAYER > POSITIONS |
| Pitch map with selectable zones |
| Natural: AM R | Accomplished: M R, ST | Competent: AM C |
| Preferred foot: Right | Other foot: Reasonable |
```
...
```typescript
interface PlayerPositionsModel {
  readonly playerId: string;
  readonly positionFamiliarities: readonly PositionFamiliarity[];
  readonly sidePreferences: readonly SidePreference[];
  readonly footedness: FootednessSummary;
  readonly roleSuitabilities?: readonly RoleSuitability[];
...
- Position familiarity is distinct from tactical role suitability
- Knowledge restrictions apply to role estimates
- Pitch colors require textual equivalents
- No position change is made directly from this read screen
...
1. Position familiarity is distinct from tactical role suitability
2. Knowledge restrictions apply to role estimates
3. Pitch colors require textual equivalents
4. No position change is made directly from this read screen
...
```text
Implement Player Positions for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player positions screen
```
