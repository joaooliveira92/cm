...
Player Happiness presents morale, concerns, promises, relationships, playing-time expectations, contract satisfaction, transfer wishes, and resolution progress.

## 2. Primary user goals

- Open a concern or promise
- Start an authorized player interaction
- Review playing-time and contract context
- Navigate to relevant match, transfer, or squad decisions
...
```text
| PLAYER > HAPPINESS | Morale: Good |
| Concern: Wants more first-team football |
| Promise: Start more league matches, review 1 April |
| Relationship with manager: Stable |
```
...
```typescript
interface PlayerHappinessModel {
  readonly playerId: string;
  readonly moraleBand: string;
  readonly concerns: readonly PlayerConcernSummary[];
  readonly promises: readonly PlayerPromiseSummary[];
  readonly relationshipSummary?: RelationshipSummary;
  readonly visibility: HappinessVisibilityPolicy;
...
- Do not expose hidden numeric morale formulas
- Concerns need committed factual reasons
- Promises require explicit terms and deadlines
- Private interpersonal data is visible only to authorized managers
...
1. Do not expose hidden numeric morale formulas
2. Concerns need committed factual reasons
3. Promises require explicit terms and deadlines
4. Private interpersonal data is visible only to authorized managers
...
```text
Implement Player Happiness for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player happiness screen
```
