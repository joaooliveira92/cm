...
Player Contract shows current employment terms, dates, wage, bonuses, clauses, squad status, loan terms, expiry, and negotiation availability according to viewer permission.

## 2. Primary user goals

- Open negotiation when authorized
- Review clause definitions
- Open loan agreement or future contract
- Navigate to club finances and transfer status
...
```text
| PLAYER > CONTRACT | North United |
| Start 1 Jul 2003 | Expiry 30 Jun 2007 | Wage 18k EC/week |
| Squad status: First team | Clauses and bonuses |
| [Offer New Contract] [Transfer Status] |
```
...
interface PlayerContractModel {
  ...
  readonly wage?: Money;
  readonly statusProfileId: string;
  readonly bonuses: readonly ContractBonus[];
  readonly clauses: readonly ContractClause[];
  readonly visibility: ContractVisibilityPolicy;
...
- Money requires explicit currency and period
- Confidential terms obey visibility policy
- Contract preview data must not be treated as an active negotiation
- Expired, expiring, future, loan, youth, and noncontract states are distinct
...
1. Money requires explicit currency and period
2. Confidential terms obey visibility policy
3. Contract preview data must not be treated as an active negotiation
4. Expired, expiring, future, loan, youth, and noncontract states are distinct
...
```text
Implement Player Contract for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player contract screen
```
