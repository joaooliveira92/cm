...
Player Form presents recent match performances, ratings, minutes, goals, assists, cards, condition, and role context across a selected period and competition scope.

## 2. Primary user goals

- Filter by competition, club, home or away, and period
- Open a match and player performance detail
- Sort supported columns
- Compare recent form with season form
...
```text
| PLAYER > FORM | Last 10 matches |
| Date Opponent Pos Min Rating Goals Assists Cards |
| 10 Feb Harbour DC 90 7.4 0 0 - |
| Average rating 7.12 | Minutes 842 |
```
...
interface PlayerFormModel {
  ...
  readonly period: FormPeriod;
  readonly rows: readonly PlayerMatchFormRow[];
  readonly aggregates: PlayerFormAggregates;
  readonly detailAvailability: MatchDetailAvailability;
...
- Ratings may be unavailable for results-only competitions
- Substitute appearances and unused bench entries are distinct
- Abandoned and awarded matches need explicit rules
- Aggregates use only eligible rows and disclose exclusions
...
1. Ratings may be unavailable for results-only competitions
2. Substitute appearances and unused bench entries are distinct
3. Abandoned and awarded matches need explicit rules
4. Aggregates use only eligible rows and disclose exclusions
...
```text
Implement Player Form for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player form screen
```
