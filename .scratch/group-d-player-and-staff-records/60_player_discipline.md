...
Player Discipline presents bookings, dismissals, bans, accumulated-card thresholds, fines or club discipline, appeals, and competition-specific eligibility.

## 2. Primary user goals

- Filter discipline by season and competition
- Open incident, match, or suspension detail
- Start an appeal or club-discipline workflow when permitted
- Review next eligibility date
...
```text
| PLAYER > DISCIPLINE |
| Competition Cards Bans Threshold Next eligible |
| First Division 4Y 0R None 1 card 16 Feb |
| Current suspension: National Cup, 1 match |
```
...
```typescript
interface PlayerDisciplineModel {
  readonly playerId: string;
  readonly competitionSummaries: readonly CompetitionDisciplineSummary[];
  readonly activeSuspensions: readonly SuspensionSummary[];
  readonly clubActions: readonly ClubDisciplinaryAction[];
...
- Domestic, continental, and international bans remain distinct
- Card thresholds are competition-rule driven
- Appeals do not suspend a ban unless rules say so
- Historical corrections require audited events
...
1. Domestic, continental, and international bans remain distinct
2. Card thresholds are competition-rule driven
3. Appeals do not suspend a ban unless rules say so
4. Historical corrections require audited events
...
```text
Implement Player Discipline for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player discipline screen
```
