...
Player Injuries shows current and historical injuries, severity, diagnosis confidence, treatment, rehabilitation, expected availability, recurrence risk, and medical recommendations.

## 2. Primary user goals

- Review diagnosis and rehabilitation status
- Open treatment decision workflow when authorized
- View injury history and recurrence context
- Navigate to medical staff or availability schedule
...
```text
| PLAYER > INJURIES | Current: Hamstring strain |
| Severity: Moderate | Estimated return: 2-4 weeks |
| Treatment: Rehabilitation | Medical confidence: Good |
| Injury history and availability timeline |
```
...
```typescript
interface PlayerInjuryModel {
  readonly playerId: string;
  readonly currentInjuries: readonly InjuryEpisode[];
  readonly history: readonly InjuryEpisodeSummary[];
  readonly medicalAssessment: MedicalAssessmentSummary;
  readonly availabilityForecast?: AvailabilityForecast;
  readonly visibility: MedicalVisibilityPolicy;
...
- Return dates are estimates and ranges
- Medical confidence must be disclosed
- Do not provide real-world medical advice
- Health information is sensitive and permission-restricted
...
1. Return dates are estimates and ranges
2. Medical confidence must be disclosed
3. Do not provide real-world medical advice
4. Health information is sensitive and permission-restricted
...
```text
Implement Player Injuries for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player injuries screen
```
