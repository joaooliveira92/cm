...
Player Statistics provides season, competition, club, and career aggregates for appearances, starts, substitute entries, minutes, goals, assists, cards, ratings, clean sheets, and position-specific metrics.

## 2. Primary user goals

- Change season, club, competition, and aggregate scope
- Inspect metric definitions
- Open the competition or season
- Export a permitted statistics summary
...
```text
| PLAYER > STATISTICS | Season 2004/05 |
| Competition Apps Starts Min Goals Ast Rating |
| First Division 24 22 1980 8 6 7.16 |
| Career totals and position-specific metrics |
```
...
```typescript
interface PlayerStatisticsModel {
  readonly playerId: string;
  readonly seasonId?: string;
  readonly scopes: readonly PlayerStatisticScope[];
  readonly rows: readonly PlayerStatisticRow[];
  readonly metricDefinitions: readonly StatisticDefinition[];
  readonly dataCoverage: StatisticCoverage;
}
```
...
- Starts, substitute appearances, and total appearances must be consistent
- Metrics unavailable at low simulation detail remain unavailable
- Career totals must not double-count transfers within a season
- Position-specific metrics need explicit denominators
...
1. Starts, substitute appearances, and total appearances must be consistent
2. Metrics unavailable at low simulation detail remain unavailable
3. Career totals must not double-count transfers within a season
4. Position-specific metrics need explicit denominators
...
```text
Implement Player Statistics for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player statistics screen
```
