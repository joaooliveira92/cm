...
The Player Profile is the canonical landing page for a footballer. It combines identity, club, availability, position, condition, morale, recent form, contract, transfer state, and permission-aware actions.

## 2. Primary user goals

- Open profile tabs and linked club, nation, competition, or fixture
- Open the contextual Player Action Menu
- Add or remove the player from a shortlist
- Navigate to previous or next player within a stable source list
...
```text
| PLAYER NAME | Club | Position | Availability |
| Overview | Attributes | Form | Statistics | History | Contract |
| Condition 94% | Morale Good | Form 7.12 | Value 2.4m EC |
| Key summary, status alerts, next fixture, current role |
```
...
interface PlayerProfileModel {
  ...
  readonly employment: PlayerEmploymentSummary;
  readonly availability: PlayerAvailabilitySummary;
  readonly position: PlayerPositionSummary;
  readonly form?: PlayerFormSummary;
  readonly contract?: PlayerContractSummary;
  readonly transfer?: PlayerTransferSummary;
  readonly permittedActions: readonly PlayerAction[];
}
```
...
- Knowledge-limited values must remain Unknown or approximate
- Condition, fitness, injury, suspension, registration, and selection are separate states
- Value is an estimate, not a guaranteed transfer fee
- Portraits and biographical data require original or licensed assets
...
1. Knowledge-limited values must remain Unknown or approximate
2. Condition, fitness, injury, suspension, registration, and selection are separate states
3. Value is an estimate, not a guaranteed transfer fee
4. Portraits and biographical data require original or licensed assets
...
```text
Implement Player Profile for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player profile screen
```
