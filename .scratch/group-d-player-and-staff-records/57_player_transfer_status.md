# Screen 57: Player Transfer Status
...
Player Transfer Status manages or displays availability for transfer, loan, squad role, asking price, club instructions, interest, and future movement.

## 2. Primary user goals

- Change listing and asking price when authorized
- Set offer-to-clubs instructions
- Review declared interest and active offers
- Open transfer or loan negotiation workflows
...
```text
| PLAYER > TRANSFER STATUS |
| Transfer list: No | Loan list: Available |
| Asking price: Not set | Squad role: Rotation |
| Interested clubs: 3, subject to visibility |
```
...
```typescript
interface PlayerTransferStatusModel {
  readonly playerId: string;
  readonly clubId?: string;
  readonly transferAvailability: string;
  readonly loanAvailability: string;
  readonly askingPrice?: Money;
  readonly squadStatusId?: string;
  readonly instructions: readonly TransferInstruction[];
  readonly interestSummaries: readonly ClubInterestSummary[];
  readonly revision: number;
  readonly permittedActions: readonly TransferStatusAction[];
}
```
...
- Listing does not create an offer or completed transfer
- Player wishes and club instructions are separate
- Interest confidence and source must be visible
- Every mutation requires transfer authority, revision validation, and budget-safe rules
...
1. Listing does not create an offer or completed transfer
2. Player wishes and club instructions are separate
3. Interest confidence and source must be visible
4. Every mutation requires transfer authority, revision validation, and budget-safe rules
...
```text
Implement Player Transfer Status for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player transfer status screen
```
