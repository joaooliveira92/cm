# Screen 122: Scouting Priorities
...
Scouting Priorities ranks the club’s recruitment needs by position, role, age profile, squad status, budget, timing, and strategic importance.

## 2. Primary user goals

- Create, rank, edit, pause, complete, or archive priorities
- Link a priority to squad-planner gaps
- Assign scouts and recruitment focuses
- Review candidates and progress
- Apply a previewable recommendation order
...
```text
+------------------------------------------------------------------------------+
| Priority list, squad need, role profile, budget, deadline, progress, owner a |
|------------------------------------------------------------------------------|
...
```typescript
interface ScoutingPriorityDraft {
...
- Priority rank does not force a transfer decision
- Budget bands are planning constraints rather than reserved money unless policy says so
- Several priorities may target the same player without duplication
- Recommendation ordering must be explainable
...
1. Priority rank does not force a transfer decision
2. Budget bands are planning constraints rather than reserved money unless policy says so
3. Several priorities may target the same player without duplication
4. Recommendation ordering must be explainable
...
```text
Implement Scouting Priorities for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify scouting priorities screen
```
