# 04: The vitest config documents the wrong default — 100 files spell the split out by hand
...
- **100 of 144** renderer test files carry the pragma
...
100 times, and silently absent on the 101st. That absence is undetectable until a renderer helper
...
It flips 44 renderer files from node to jsdom in one move. At least one of them,
...
Acceptance:
- [ ] `vitest.config.ts` declares the two projects; renderer defaults to jsdom, main to node
- [ ] Redundant per-file pragmas removed
- [ ] A renderer test file added with no pragma gets a DOM — proved by a test, not by inspection
- [ ] Full desktop suite run before and after, with the set-diff of failing files stated; no file that
