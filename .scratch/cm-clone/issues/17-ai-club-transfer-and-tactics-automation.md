...
- [x] At each window's open, an AI club identifies Positions where its best Position Rating falls
      below a fixed threshold relative to the league average for that slot
- [x] For each gap, the AI club targets the highest-Transfer-Value affordable player (Natural/
...
- [x] If countered, the AI club accepts up to 1.15x Transfer Value if still affordable, otherwise
      withdraws
- [x] An AI club selling a player accepts bids ≥1.0x Transfer Value outright, counters bids between
      0.85x–1.0x up to exactly Transfer Value, and rejects bids below 0.85x, with no squad-need veto
- [x] At season start, each AI club is assigned one fixed Tactic: formation chosen to maximize
...
- [x] AI club Tactics never change mid-season or react to opponent/in-match state
- [x] All AI-club activity is invoked in-process and never touches the RpcGroup
