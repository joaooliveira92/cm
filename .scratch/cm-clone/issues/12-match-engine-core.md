...
- [x] Three Phase Strengths (Attack, Midfield, Defense) computed per team pre-tactics from Position
      Ratings of players occupying each phase's Positions
- [x] Tactic resolution produces a flat `TacticalModifiers` struct (attack/midfield/defense/tempo/
...
- [x] Home advantage (~+5–10%) applied to home Phase Strengths before Tactical Modifiers
- [x] Fatigue decay from minute ~60 on Midfield/Defense Phase Strength, scaled inversely by
      squad-average Stamina, reset per match
- [x] 90 Minute-Slices plus a Stoppage Slice per half (1–5 minutes, biased by that half's
      stoppage-causing event count) resolve a full match
- [x] Full Match Event vocabulary emitted: `MatchStarted` (with seed), `Goal`, `ShotOnTarget`,
      `ShotMissed`, `BigChance`, `YellowCard`, `RedCard`, `Injury`, `Substitution`,
      `HalfTimeReached`, `FullTimeWhistle`
- [x] Given the same `MatchStarted` seed and the same commands, replaying produces an identical event
      timeline (test asserts this directly)
- [x] `ChangeTactics` and `MakeSubstitution` are accepted as valid mid-match commands by the Decider
