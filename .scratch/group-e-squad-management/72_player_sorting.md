...
Player Sorting defines stable single-column or multi-column ordering for squad and selection lists while preserving uncertainty, unavailable values, and user-defined tie-breakers.

## 2. Primary user goals

- Sort by a column header
- Build a multi-level sort
- Reverse or remove a sort level
- Restore a view-specific default
- Save sorting as part of a custom squad view
...
| SORT PLAYERS                                                                  |
|------------------------------------------------------------------------------|
| 1. Selection status                 Ascending [v]                             |
| 2. Natural position                 Ascending [v]                             |
| 3. Condition                        Descending [v]                            |
| Tie-breaker: Player display name                                             |
| [Add Level] [Reset] [Cancel] [Apply]                                         |
+------------------------------------------------------------------------------+
```
...
```typescript
interface PlayerSortDefinition {
  readonly levels: readonly PlayerSortLevel[];
  readonly unknownValuePolicyId: string;
  readonly localeId: string;
  readonly stableTieBreakerId: string;
...
- Sorting must not leak hidden exact attributes
- Stable tie-breakers prevent row jumping
- Locale-aware name sorting preserves structured names
...
1. Unknown values do not become zero
2. Sorting must not leak hidden exact attributes
3. Stable tie-breakers prevent row jumping
4. Locale-aware name sorting preserves structured names
...
```text
Implement Player Sorting for an original football-management simulation. Use stable
...
```text
docs(game-ui): specify player sorting screen
```
