...
const handlers: { readonly [M in AppRpcMethod]: Handler<M> } = {
  ...
  getCompetitionTable: (payload, ctx) =>
    Effect.gen(function* () {
  ...
  getAwaitingMatch: (payload, ctx) =>
    Effect.gen(function* () {
      const { saveId, matchId } = yield* Schema.decodeUnknownEffect(AppRpcs.getAwaitingMatch.payload)(payload);
      return yield* getAwaitingMatch(ctx.savesDir, saveId, matchId);
    }),
  ...
  getPostMatchSummary: (payload, ctx) =>
    Effect.gen(function* () {
      const { saveId, matchId } = yield* Schema.decodeUnknownEffect(AppRpcs.getPostMatchSummary.payload)(payload);
      return yield* getPostMatchSummary(ctx.savesDir, saveId, matchId);
    }),
  getMatchStatistics: (payload, ctx) =>
    Effect.gen(function* () {
  ...
  getBudgetReviewScreen: (payload, ctx) =>
    Effect.gen(function* () {
      const { saveId } = yield* Schema.decodeUnknownEffect(AppRpcs.getBudgetReviewScreen.payload)(payload);
      return yield* getBudgetReviewScreen(ctx.savesDir, saveId);
    }),
  getTransferHistoryScreen: (payload, ctx) =>
    Effect.gen(function* () {
      const { saveId } = yield* Schema.decodeUnknownEffect(AppRpcs.getTransferHistoryScreen.payload)(
        payload,
      );
      return yield* getTransferHistoryScreen(ctx.savesDir, saveId);
    }),
  ...
  getPlayerSearch: (payload, ctx) =>
    Effect.gen(function* () {
      const { saveId, query } = yield* Schema.decodeUnknownEffect(AppRpcs.getPlayerSearch.payload)(
        payload,
      );
