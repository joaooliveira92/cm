...
test("creating a save with an empty name produces no save and no crash", async ({ window, userDataDir }) => {
...
test("duplicate save names are allowed and both appear in the continue list", async ({ window, userDataDir }) => {
  const input = window.getByPlaceholder("Save name");
  const create = window.getByRole("button", { name: "Create" });
...
test("clicking a stale save entry (file deleted) is a silent no-op — stays on landing screen", async ({ window, userDataDir }) => {
